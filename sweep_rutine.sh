...
#	python3 growNet.py --size 500 --S_0 1 --S_1 4 --ratio 0.34 --replacement 0.2 >/dev/null
	...
	./ising_model --J_ij="{0, 4.40, 0}" --D_i="{0, 0}" --T=6 --init=sat --out=output --seed=$1 > output.txt
#	echo "$2 $(python3 average.py -s --trim --T $val --size 25)" >> ../susceptibility.txt
	python3 plot.py --savefig --trim --label "N=$int_val"
